...
rm -r ${workdir}/IndAssembly/Megahit
fi

#Co-assembly
now=$(date +"%Y-%m-%d %H:%M:%S")
echo "$now | Individually assembling all reads" >> ${workdir}/run_${timestamp}.log
if [[ $samplefile =~ "/" ]]; then
	#It is PE
  while read sample; do
  samplename=$(echo $sample | cut -d ' ' -f1 )
  megahit -t ${threads} -1 ${workdir}/IndAssembly/${samplename}_1.fasta -2 ${workdir}/IndAssembly/${samplename}_2.fasta -o ${workdir}/IndAssembly/Megahit/${samplename}
  done < ${sampledatafile}
else
	#It is SR
  while read sample; do
  samplename=$(echo $sample | cut -d ' ' -f1 )
	megahit -t ${threads} -r ${workdir}/IndAssembly/${samplename}.fasta -o ${workdir}/CoAssembly/Megahit/${samplename}
  done < ${sampledatafile}
fi
now=$(date +"%Y-%m-%d %H:%M:%S")
echo "$now | Individual assembly succesfully finished" >> ${workdir}/run_${timestamp}.log

#Merging assemblies

#Concatenate assemblies
all_assemblies=$(ls -p ${workdir}/IndAssembly/Megahit/*/final.contigs.fa)

#Merge assemblies
wtdbg2 -i ${all_assemblies} -t ${threads} -x ont -g 5G -fo ${workdir}/IndAssembly/Megahit/final.contigs.merged
