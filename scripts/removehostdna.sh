...
echo "$now | 		Removing host DNA from PE data from folder ${sourcefolder}" >> ${workingdirectory}/${project}/run.log
	...
	while read sample; do
		...
		perl ${workingdirectory}/scripts/repairreads.pl -f1 ${workingdirectory}/${project}/${sourcefolder}/${samplename}_1.fastq -f2 ${workingdirectory}/${project}/${sourcefolder}/${samplename}_2.fastq -r '^@(\\S+):[1|2]\$' -t -o {workingdirectory}/${project}/HostDNARemoved/${samplename}
