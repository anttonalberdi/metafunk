...
while read sample; do
		...
		if [[ $sampleinfo =~ "/" ]]; then
			...
			bwa mem -t ${threads} -R '@RG\tID:ProjectName\tCN:AuthorName\tDS:Mappingt\tPL:Illumina1.9\tSM:Sample' ${workdir}/HostDNARemoved/ReferenceGenomes/${genomefile} ${workdir}/HostDNARemoved/${samplename}_1.fastq ${workdir}/HostDNARemoved/${samplename}_2.fastq | samtools view > ${workdir}/HostDNARemoved/${samplename}.sam
			#| grep -v -P '^@|NH:i:[0-2]\b'| samtools view -b - > ${workdir}/HostDNARemoved/${samplename}.bam
